...
const Schedules = () => {
  ...
  const filteredSchedules = schedules.filter((s) => {
    ...
    return (
      (bus?.plateNumber?.toLowerCase() || "").includes(
        searchTerm.toLowerCase(),
      ) ||
      (route?.departureLocation?.toLowerCase() || "").includes(
        searchTerm.toLowerCase(),
      ) ||
      (route?.destinationLocation?.toLowerCase() || "").includes(
        searchTerm.toLowerCase(),
      )
    );
  });
    ...
    <Layout>
      <div className="space-y-6 max-w-7xl mx-auto">
        ...
        {loading ? (
          ...
          <div
            className="p-12 text-center bg-white rounded-[16px]"
            style={{ boxShadow: "0 2px 16px rgba(108,99,255,0.07)" }}
          >
            <Calendar
              className="w-14 h-14 mx-auto mb-3"
              style={{ color: "#6B7280", opacity: 0.4 }}
            />
          ...
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {filteredSchedules.map((schedule) => {
              ...
              const driver = drivers.find((d) => d.id === schedule.driver?.id);
              return (
                <div
                  key={schedule.id}
                  className="bg-white rounded-[16px] p-5 hover:-translate-y-1 transition-all duration-200"
                  style={{ boxShadow: "0 2px 16px rgba(108,99,255,0.07)" }}
                >
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="flex gap-1">
                      <button
                        ...
                        className="p-2 rounded-lg hover:bg-[#EEF0FF] transition-colors"
                      >
                        <Edit
                          className="w-4 h-4"
                          style={{ color: "#6B7280" }}
                  ...
                  <div className="space-y-2 text-xs">
                    ...
                    <div className="flex items-center gap-2 text-[#6B7280]">
                      ...
                      <span>
                        {route?.departureLocation || "N/A"} →{" "}
                        {route?.destinationLocation || "N/A"}
                      </span>
                    </div>
                    <div className="flex items-center gap-2 text-[#6B7280]">
        ...
        {showModal && (
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div
              className="bg-white rounded-[16px] p-7 w-full max-w-md"
              style={{ boxShadow: "0 8px 40px rgba(108,99,255,0.18)" }}
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="text-xs font-bold text-[#6B7280] uppercase tracking-widest mb-1.5 block">
                    Departure Time
                  </label>
                  <input
                    type="datetime-local"
